...
케릭터가 있는 장소는 1 x 1 크기의 정사각형으로 이뤄진 N x M 크기의 직사각형으로, 각각의 칸은 육지 또는 바다이다.
케릭터는 동서남북 중 한 곳을 바라본다.

맵의 각 칸은 (A, B)로 나타낼 수 있고, A는 북쪽으로부터 떨어진 칸의 개수, B는 서쪽으로부터 떨어진 칸의 개수이다.
캐릭터는 상하좌우로 움직일 수 있고, 바다로 되어 있는 공간에는 갈 수 없다.
캐린터의 움직임을 설정하기 위해 정해 놓은 매뉴얼을 이러하다.
...
방향 d의 값으로는 다음과 같이 4가지가 존재한다. 0: 북쪽 / 1: 동쪽 / 2: 남쪽 / 3: 서쪽
- 셋째 줄부터는 맵이 육지인지 바다인지에 대한 정보가 주어진다. 
